    ...
    return (
      <div className="animate-fade-in pb-10">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-cute text-purple-900">添加管理员</h2>
          ...
          <div className="ml-auto flex gap-3">
            ...
            <button onClick={handleAdd} className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-purple-500 to-indigo-500 text-white font-bold text-sm shadow-lg shadow-purple-200 flex items-center gap-2"><Plus size={16} /> 创建管理员</button>
          </div>
        </div>
        <div className="solid-card p-8 rounded-3xl space-y-6 max-w-2xl mx-auto">
          <h3 className="font-cute text-lg text-purple-900 flex items-center gap-2 mb-2"><span className="w-1.5 h-4 bg-yellow-400 rounded-full"></span> 管理员信息</h3>
          <div>
          ...
          <div>
            <label className="block text-xs font-bold text-purple-800 mb-2">初始密码</label>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-cute text-purple-900 flex items-center gap-2"><span className="w-1.5 h-6 bg-yellow-400 rounded-full inline-block"></span>管理员账号</h2>
          <p className="text-gray-400 text-xs mt-1 ml-4">管理后台管理员列表、禁用与密码修改</p>
        </div>
        <button onClick={() => setView('create')} className="bg-purple-500 text-white px-4 py-2 rounded-xl font-bold shadow-lg shadow-purple-200 hover:bg-purple-600 transition-colors flex items-center gap-2 text-xs"><Plus size={14} /> 添加管理员</button>
      </div>
          ...
          <tbody className="text-sm divide-y divide-purple-100">
            {admins.map(a => (
              <tr key={a.id} className="hover:bg-purple-50/30 transition-colors">
                ...
                <td className="p-3 font-bold text-purple-700">{a.username}</td>
          ...
          <div className="fixed top-0 right-0 bottom-0 left-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl border border-purple-100 p-6 w-full max-w-md shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-cute text-lg text-purple-900">修改密码</h3>
                <button onClick={() => setPwdTarget(null)} className="text-gray-400 hover:text-gray-600 transition-colors"><X size={20} /></button>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-purple-800 mb-1.5">新密码</label>
              ...
              <div className="flex justify-end gap-3 mt-8">
                ...
                <button onClick={handleChangePwd} className="px-6 py-2.5 rounded-xl bg-purple-500 text-white font-bold text-sm shadow-lg shadow-purple-200 hover:bg-purple-600 transition-all">确认修改</button>
              </div>
            </div>
          </div>
        </>
