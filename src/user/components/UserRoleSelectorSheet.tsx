      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            <motion.div
              key="sheet"
              className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-[0_-10px_30px_rgba(0,0,0,0.08)] will-change-transform transform-gpu"
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              exit={{ y: '100%' }}
            ...
            >
              ...
              <div className="p-4 flex flex-col gap-3 max-h-[70vh] overflow-y-auto">
                {roles.map(r => (
                  <div key={r.id} onClick={() => handleSelect(r)} className={`p-4 bg-white rounded-xl shadow-sm border border-slate-100 flex items-center gap-3 active:scale-[0.98] transition-transform ${selected === r.id ? 'border-[#A855F7] bg-[#FBF5FF]' : ''}`}>
