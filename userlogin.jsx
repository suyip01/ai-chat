...
export default function LoginPage() {
    ...
    const [email, setEmail] = useState('itsmemamun1@gmail.com');
    ...
    return (
        <div className="min-h-screen w-full flex items-center justify-center bg-[#F3E5F5] relative overflow-hidden font-sans text-[#4A4060]">
            ...
            {/* Ambient Background (Replicating the reference style) */}
            ...
            {/* Main Container 
        - Mobile: Full width, transparent feel (content flows naturally)
        - Desktop: Card style with shadow and blur
      */}
                ...
                <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                    ...
                    <div className="group relative transition-all duration-300">
                        ...
                        <div className="absolute right-4 top-1/2 transform -translate-y-1/2 text-purple-500 pointer-events-none transition-opacity duration-300">
                            {/* Only show check if email has some length (mock validation) */}
                            {email.length > 0 && <Check size={20} strokeWidth={2.5} />}
                        </div>
                    </div>
                    ...
                    <button
                        type="submit"
                        className="w-full bg-gradient-to-r from-purple-500 to-indigo-500 hover:opacity-90 text-white font-bold py-4 rounded-2xl shadow-lg shadow-purple-200 active:scale-[0.98] transition-all duration-200 ease-out text-lg mt-4 tracking-wide"
                    >
                        Login
                    </button>
            ...
            {/* Decorative Blur Blobs (Updated to Purple/Gold/Pink theme) */}
