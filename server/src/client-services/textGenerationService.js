...
export class TextGenerationService {
  ...
  async generateResponse(history, latestUserText, systemPromptOverride, modelOverride, temperatureOverride, context = {}) {
    ...
    const reqTail = (() => { const m = [...messages].reverse().find(x => x && x.role === 'user' && typeof x.content === 'string'); return m ? String(m.content).slice(-20) : ''; })();
    ...
    for (let i = 0; i <= this.retryCount; i++) {
      try {
        ...
        try { console.log('\x1b[31mLLM OUTPUT CONTENT\x1b[0m', out, '\x1b[31mREQ TAIL\x1b[0m', reqTail); } catch {}
